    ...
    def schema(self) -> ToolSchema:
        return ToolSchema(
            ...
            description=(
                ...
                "same query, and never invent results. Use this for ONE "
                "search only. If the request is a chain of steps — e.g. "
                "'find any new files AND tell me what's in them', 'search "
                "my notes for X then read the best match' — call "
                "start_workflow instead and hand it the whole goal; do NOT "
                "hand-run a multi-step job by calling this and then "
                "start_file_read yourself. Returns JSON: "
    ...
    def schema(self) -> ToolSchema:
        return ToolSchema(
            ...
            description=(
                ...
                "and never invent the content. Use this for ONE file only. "
                "If the request is a chain of steps — e.g. 'find the "
                "newest report AND summarise it', 'dig through my notes "
                "for X then read the best one' — call start_workflow "
                "instead and hand it the whole goal; do NOT hand-run a "
                "multi-step job by chaining start_file_search and this "
                "tool yourself. If the path is ambiguous "
