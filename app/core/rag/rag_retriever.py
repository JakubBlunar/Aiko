...
class RagRetriever:
    ...
    def recall_concept(
        ...
        *,
        ...
        # Higher-order links this concept participates in, so the "why" can
        # reach past raw memories to the concepts it connects to: meta concepts
        # that reference it (a tension it sits in, an L20 generalization that
        # abstracts it), and its own base concepts when it is itself a meta.
        # Bounded so the bundle stays compact.
    ...
    def _concept_related_links(
        ...
        ``recall_concept`` "why" bundle: the metas that reference it as a base
        (a tension it sits in, or a generalization that abstracts it), and its
        own base concepts when it is itself a meta (from the already-fetched
        ``evidence_edges``). Both L12 tensions and L20 generalizations link to
        their bases via ``("concept", id)`` ``evidence`` edges, so a
        generalization is recognised by the neighbour's *kind*, not the edge
        relation -- a generalization link is labelled ``"generalizes"`` so the
        "why" reads plainly. Skips missing / non-active neighbours; bounded by
        ...
        self_is_generalization = getattr(concept, "kind", "") == "generalization"
        ...
        try:
            # Metas that reference this concept as a base. A generalization
            # abstracting it reads as "generalizes"; a tension (or anything
            # else) keeps its own kind as the relation word.
            for dep_id in store.dependents_of(self_id):
                dep = store.get(dep_id)
                if dep is not None and dep.kind == "generalization":
                    relation = "generalizes"
                else:
                    relation = dep.kind if dep is not None else "references"
                add(dep_id, relation)
            # This concept's own base concepts, when it is a meta. When it is a
            # generalization those bases are what it generalizes *over*.
            base_relation = (
                "generalizes" if self_is_generalization else "references"
            )
            for e in evidence_edges:
                if e.src_type == "concept":
                    try:
                        add(int(e.src_id), base_relation)
                    except (TypeError, ValueError):
                        continue
        except Exception:
