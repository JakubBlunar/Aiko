...
The daylight gate uses :func:`app.core.circadian.compute` so it
...
class GardenVisitWorker:
    ...
    def run(self) -> dict[str, Any]:
        from app.core.world_store import promote_stage  # local to avoid cycle
    ...
    def _current_period(self, now: datetime) -> str:
        if self._circadian_period_provider is not None:
            try:
                ...
                pass
        try:
            from app.core.circadian import compute
