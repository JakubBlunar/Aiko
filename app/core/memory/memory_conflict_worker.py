...
2. **All-pairs cosine** within the ``[similarity_min,
   ...
   pair, ever.
3. **Heuristic gate**
   (:mod:`app.core.memory.conflict_heuristics`):
...
4. **LLM verification** for borderlines, gated by
   :class:`app.core.memory.fact_check_rate_limiter.FactCheckRateLimiter`
    ...
    def _verify_with_llm(
        ...
        try:
            stream = self._ollama.chat_stream(
                ...
                format_json=True,
                # Deciding whether two memories truly contradict (vs. just
                # being topically close) is exactly the borderline call
                # reasoning improves. Headroom added client-side.
                think=True,
