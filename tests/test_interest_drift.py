...
class _Host(InnerLifePart2Mixin):
    ...
    def _topic_gate_options(self):
        ...
        Stubbed rather than inherited because this fake deliberately has
        no cue store, so it exercises the kv-ring fallback path. The
        shipped default is used so these fixtures are checked against the
        strictness that actually runs.
        """
