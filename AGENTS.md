...
- **Persona** (`data/persona/aiko_companion.txt`) is user-editable and is always-on prompt text; every user-name reference must stay the literal `{user_name}` placeholder (any other `{…}` token crashes `.format()`). Per-cue handling notes do **not** go here — they live in `data/persona/cue_handling.txt`, are hoisted into T6 only on the turns their cue fires, and their headers must match `CuePolicy.handling_section` byte-for-byte (a mismatch fails silently). See [`docs/cue-pool.md`](docs/cue-pool.md#the-persona-hoist).
