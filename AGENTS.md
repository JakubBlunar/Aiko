...
- **When adding a TTS engine**, add it to the catalogue in `app/tts/registry.py` and get playback from `PcmPlaybackMixin` — never reimplement chunking, pacing, gain, loudness matching or the amplitude pacer. Availability must be answered *without importing the engine* (filesystem probe), because Chatterbox pins `torch==2.6.0` against the app's `2.10.0` and importing pocket-tts costs ~0.6–1 GB of PyTorch. Anything engine-independent (reaction→speed, for instance) belongs in a shared module so a provider swap cannot change her character.
