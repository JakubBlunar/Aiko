...
- `assistant.tts_length_scale` *(float, `1.0`)* — global TTS speed multiplier, clamped to `[0.65, 1.35]` here and to `[0.85, 1.15]` at the engine, which is the band that survives stacking with reaction speed and the cadence layer's per-sentence hint. **Higher → slower** speech (more "pacing"); lower → faster. Independent of any per-reaction speed jitter (`agent.tts_runtime_speed_enabled`) — it applies whether that gate is on or off, being a deliberate preference rather than affect drift. Until Aug 2026 it was implemented on pocket-tts only, and since the wiring reaches it through `getattr` it did **nothing at all** on Chatterbox; both engines now resolve it through `reactions.resolve_playback_speed()`. One caveat on tuning it: before `tts.speech_rate_match_limit` existed, a generative engine's own tempo varied ~24% per clip, so this slider was being tuned against a moving target — it was raised to 1.08 to tame sentences that happened to draw fast, which then made the slow draws worse. With tempo matched it means what it says, and `1.0` now lands on her measured incumbent pace, so re-tune from there rather than from an older value.
...
- `llm.providers[].think_num_predict_headroom` *(int, `8192`)* — Ollama-only: extra `num_predict` budget added automatically on `think=True` worker calls so the reasoning trace doesn't starve the answer. Was `2048`, which measurement showed was far too small: a 27B reasoner spends 1.6k–5.1k tokens (median 2.4k) thinking about a routine extraction, so the trace ate the whole cap and the answer came back **empty** — 98 such calls across 10 worker surfaces in ten days of log, and two workers that consequently wrote nothing for two months (H27). It's a ceiling, not a spend: raising it costs nothing on calls that finish early. If a surface still comes back empty at the cap, the client retries it once with thinking off — grep `thinking starved` to see which. See [`idle-workers.md`](idle-workers.md#the-answer-budget-when-a-worker-thinks).
...
- `agent.cue_topic_stoplist` *(bool, `true`)* — drop function words before the lexical arm of the topic gate decides a cue is on-topic, and count her name and yours as function words too. On is a **9× tightening**: admission goes 32.3% → 3.6% of cue/message pairs, because "and", "the", "that" and `aiko` were carrying most of the matches — the 580 hits on her own name came from cue subjects written *about* the two of you while he addresses her by name constantly. What it drops sits at median cosine **0.378** against a null of 0.384, i.e. very slightly *less* related than two random turns, which is the evidence it is noise and not reach. Shipped off for a while under H43 pending production evidence; H47 found it, as 604 expired cues that had each been rendered in front of her 1.4–2.0 times and passed over. **If she goes too quiet, prefer relaxing `agent.cue_topic_min_cosine` to 0.50** over turning this back off: that widens the semantic arm (7.3% of unrelated pairs clear 0.50, against 1.8% at 0.55) instead of readmitting function-word noise wholesale. `false` restores the pre-H47 gate exactly. See H43 and H47 in [`health.md`](personality-backlog/health.md).
...
- `agent.knowledge_enrichment_enabled` *(bool, `true`)* — master switch for the F9 worker. Off → the worker never registers, no web searches, no `knowledge` rows written. Also disables the D3 post-turn write-back, which borrows this worker's distiller: the synchronous `web_search` tool still answers, she just doesn't retain what she looked up.
...
- `memory.concept_drift_min_evidence` *(float, `0.36`, clamped `[0, 1]`)* — floor for persisting a finding as a learning event. Deliberately **shape-neutral**: it measures inertia overcome (plasticity-weighted, so equal movement in a sticky belief outranks it in a `taste` / `conduct` row) times how decisively the evidence moved, and nothing about what *kind* of change it is. It replaced a floor on `salience`, which carried a per-shape narrative prior and so held a belief *forming* to roughly twice the bar of a rewording, discarding formations on fluid beliefs before they were ever recorded. `0.36` is that old bar as `succession` experienced it, now applied to every shape equally.
...
- `memory.evolution_diary_min_salience` *(float, `0.30`, clamped `[0, 1]`)* — a junk backstop, not the selector. It reads like a significance filter and never was one: on 467 real events `0.45` admitted 96% of everything and no value in the plausible range was selective. What it actually did was select by *shape*. Which changes are worth telling is now decided per shape, in `select_page`: the dominant shape (`succession`, 79% of arrivals) may take at most half a contested page, so a belief forming or being lost still reaches the entry it shares a run with. The cap backfills rather than truncating, so a period in which nothing else happened is still narrated in full.
...
- `memory.concept_synthesis_affect_singleton_abs_valence` *(float, `0.35`, clamped `[0, 1]`)* — |valence| a *single* cluster must clear to mint or promote an affective concept without a twin. Below this the pair-floor still applies. Set so a mild-neg tender cluster (~−0.27) cannot mint while a clearly hard topic (~−0.40) can once it has enough samples.
...
Rejections persist in `kv_meta` under `concept_consolidation.verdicts`, keyed on the pair **plus a digest of both labels** with a 30-day TTL — so a stable template collision is paid for once instead of after every restart, while an L17 relabel re-opens the question. A remembered rejection also removes the pair from *nomination*, not just from the run loop: the batch is a cosine-sorted prefix, so an answered pair used to hold its place in that prefix forever and the worker stopped advancing once its top `batch_size` were answered (400 of 440 live candidates unreachable). Pairs at or above the auto-merge bar are exempt, since those need no adjudication and a stale verdict must not veto a merge.

Watch one `concept_consolidation run: scanned=… pairs=… (banded=… in_batch=… answered=…) auto_merged=… adjudicated=… merged=… rate_limited=…` INFO line per tick. `rate_limited=True` every run means the backlog exceeds the daily budget; `banded` is how many pairs the per-block band nominated and **`in_batch` is how many of those survived into the batch** — they were one number, and it read as the band working while the batch discarded all of it. `answered` counts pairs skipped at nomination as already adjudicated.

Merging two **meta** rows (a tension, a generalization) keeps the survivor's own base concepts and drops the absorbed twin's. A meta's arity means something — a tension declares `meta_min_active_bases=None`, so losing any one base makes it moot — and importing a twin's bases would leave the survivor standing on three or four, going moot on the first of them to fall dormant. Consolidation would then be quietly retiring the frictions it had just decided were worth keeping.
...
- `memory.concept_consolidation_batch_size` *(int, `40`, min `1`)* — cap on pairs **acted on** per run (not seeds scanned: discovery is a global scan since L46). Over-bar pairs fill it worst-first; the band gets a reserved share (below), because "the cut falls on the least-similar pairs, which are the ones that can wait" was written before the band existed and is false for it.
- `memory.concept_consolidation_band_reserve` *(int, `12`, min `0`)* — **H12.** Slots in `batch_size` held for banded nominations, and the reason the band did nothing for its whole life. `run` sorted every candidate together by cosine and cut at `batch_size`; a banded pair is below `merge_cosine` and an over-bar pair is above it *by construction*, so that sort is the two groups concatenated and the cut falls in the gap — measured on the live graph, all **65 nominations from all 22 blocks sat at global ranks 440–504** against a batch of 40, and not one had ever been adjudicated. Widening the batch is not the fix either: the over-bar population is 440 against a 30/day budget. The reserve is filled a block at a time, **least-served block first** — ranking it by cosine would reintroduce the cross-block comparison the band exists because it is invalid, and ranking it by name starves the tail, since 12 slots against 22 blocks means one pass does not reach everyone (that version gave the alphabetical head a permanent turn and left `relationship/value` unserved in perpetuity). "Least served" is read straight off the verdict cache, so it needs no cursor: a served block acquires answered pairs and sinks, a skipped one stays at zero and rises, and the 30-day TTL makes a settled block due again. Replayed over 30 runs of real budget, 22 of 22 blocks get a turn. The reserve is also **interleaved** through the batch rather than appended, because the real limit is the rate cap and anything past slot ~30 is cut a second time. `0` restores the old behaviour.
- `memory.concept_consolidation_merge_cosine` *(float, `0.84`, clamped `[0, 1]`)* — the bar for being a *candidate*. Was `0.88`, which admitted only 12 pairs across a month; below ~`0.82` pairs stop being restatements and start being different subjects sharing a sentence template, which the adjudicator would have to reject over and over. Since H16 this is a **ceiling** rather than the only rule — see the band below.
- `memory.concept_consolidation_candidate_floor` *(float, `0.78`, clamped to `[0, merge_cosine]`)* / `memory.concept_consolidation_block_top_n` *(int, `3`, min `0`)* — **H16.** Each `(subject, kind)` block also nominates its own top *N* pairs from the band between this floor and `merge_cosine`. A single absolute bar assumes every kind's similarity distribution sits in the same place, and measurement says otherwise: `0.84` is above the **99th percentile** for 17 of the graph's 19 blocks, and for `tension/relationship` it admits **zero** of 406 pairs — against 28 rows a reader sees as about five frictions restated. Label shape is why. A tension is two clauses ("X seeks A, yet I value B") and the longest label in the register (204–278 chars vs 62–190 elsewhere), so restating one clause moves only half the vector: two rows opening with an identical sentence reach `0.851` while two unrelated frictions reach `0.846`. Within the band, metas that **share a base concept** rank first — two tensions standing on the same belief are the same friction however differently worded, which is the judgement cosine cannot make here. `block_top_n = 0` restores the flat-bar behaviour. The cost is bounded by construction: at most `blocks × N` extra candidates per tick (57 on the live graph), each one a bounded LLM call with a 30-day cached verdict.
...
- `agent.coactivation_block_enabled` *(bool, `true`)* — L4: the co-activation prompt block, a hedged T1 (semi-stable) line naming the topics that keep lighting up together in the same conversations ("you've been circling X / Y / Z together lately") plus a cluster that's gone quiet, so Aiko carries a sense of the user's current "mode". Built from the topic graph's **session** co-activation signal only — clock-invariant, so the T1 prompt-cache prefix does not churn with time of day. Day / circadian / weekday axes exist on the graph and feed L2 synthesis plus T3 temporal priming (`ConceptView.activated` via matching-period / matching-weekday reps), not this block. No-op in the non-persistent topic-graph mode, silent while the graph is still immature (L21), dropped under aggressive context pressure.
...
**Off by default.** After a reply has already been delivered, a background job re-reads it against the very context Aiko was handed and asks two questions: was she still chewing on something (a thread she under-answered, an implication she noticed but did not follow), and was something already in front of her — a memory, a concept about {user_name} — that was relevant and got talked past? A drafted answer becomes a `second_thought` cue that a later turn can claim, which is how a thought outlives the turn that produced it. Design notes in [`second_thought_worker.py`](../app/core/proactive/second_thought_worker.py); the pool side in [`cue-pool.md`](cue-pool.md#the-second-thought-k96).

Two facts about the shape are worth knowing before tuning it, because they are what make it affordable:

- **It runs on the chat model, not the worker model** — the one inner-life worker that does. Its input is the *identical* system prefix the turn just sent (carried through `PromptTelemetry.system_prompt` + `cache_breakpoints`), so those ~74k characters are a cache **read**. Pointing it at the local worker model would be cheaper per token and blind to the context the feature exists to re-read.
- **It runs after the stream, not before it.** Measured over 1,162 turns, a pre-stream call would cost time-to-first-token — the one latency a companion cannot hide, and 54% of turns currently reach first token inside 1.5s. After the reply, it lands in the gap while {user_name} reads and types, so the thought is usually waiting before the next message arrives. Raising `llm.routes.main_chat.reasoning_effort` is the alternative that was **rejected**: that buys hidden thinking on the same input, out of the *combined* `max_output_tokens` the visible reply also draws from.

Settings (all under `agent`):

- `second_thought_enabled` *(bool, `false`)* — master switch, and the one-line revert. Off → no call is ever made and the block never renders. Deliberately dark on a fresh install: this is the only worker that spends on the main chat route.
- `second_thought_max_tokens` *(int, `160`, min `32`)* — output ceiling, and effectively the whole cost dial since the input is a cache read. The floor matters: below roughly 32 the draft truncates mid-sentence and lands in `unparsed` rather than declining cleanly.
- `second_thought_min_gap_seconds` *(int, `180`, min `0`)* — floor between calls. The pass can decline, but declining still costs a call, so the cheap clock runs first. Raise to spend less; `0` allows one per turn (bounded then only by the shelf).
- `second_thought_min_user_chars` *(int, `80`, min `0`)* / `second_thought_min_reply_chars` *(int, `120`, min `0`)* — length floors. "ok" / "thanks" / "lol" turns have nothing to have a second thought about, and these are what keep the spend sane.

Pacing and retries are **not** here — they belong to the cue policy in [`cue_accounting.py`](../app/core/proactive/cue_accounting.py): `surface_cooldown_hours=4` paces how often she actually raises one, `inventory_target=2` stops a turn-triggered producer filling the shelf, and `ttl_hours=24` lets a loose end carry into tomorrow but not into Friday.

Verification: `get_second_thought_state()` (MCP) reports the funnel — read `declined` as **health** rather than failure (most turns should need no second thought) and `unparsed` as the bug signal. `force_second_thought_draft()` replays the last turn past the clock and length floors; `force_second_thought_surface()` then bypasses the cadence so the cue lands on the next assembly. INFO logs: `second thought: subject=… ms=…` on a draft, `second-thought fire: cue=… subject=…` on a surfacing. Tests: `tests/test_second_thought.py`, `SecondThoughtProviderSlotTests` + `SecondThoughtBreakpointCarryTests` in `tests/test_prompt_assembler.py`, `SecondThoughtSettingsTests` in `tests/test_settings.py`.
...
- `agent.wants_per_source_cap` *(int, `4`, `0` disables)* — how many of those slots any one source may hold. The total cap alone is filled in arrival order and the sources do not arrive at comparable rates: curiosity seeds are offered first and never run short, so they held 7 of 8 slots while goals and pursuits — the sources carrying anything durable — could not claim one at all ([H29](personality-backlog/shipped/health.md#h29-a-want-cannot-outlive-two-showings-of-its-source-cue)). Higher → the fastest producer owns more of what she wants to raise; `0` → back to pure arrival order.
...
A want built on a curiosity seed retires when that seed's **subject is settled** — `used` (the topic came up) or `superseded` (the row was merged away) — and at no other time. Not when the seed leaves the pending shelf, which happens the moment it renders into a prompt ([H28](personality-backlog/shipped/health.md#h28-a-different-block-spent-k52s-fuel-and-three-features-starved)), and not when it *expires* either: a seed expires by being offered and refused, and every one of the 110 expired seeds on the reference install died at `max_surfacings` a median 2.9 hours after birth, against a pressure mechanic that needs 19 hours to reach the first bar ([H29](personality-backlog/shipped/health.md#h29-a-want-cannot-outlive-two-showings-of-its-source-cue)). Being shown something and not biting is the state that most deserves to keep wanting, so an expired seed leaves its want behind and the want ages on the ledger's own `wants_max_age_days` clock.
...
- `memory.restate_threshold` *(float, `0.85`, clamped `[0.5, 0.999]`)* — the second, narrower dedupe gate: the cosine floor at which a fact **restated shortly after** the first telling is merged instead of given its own row. Deliberately below `dedupe_threshold`, and only safe because three further conditions come with it — see [H31](personality-backlog/shipped/health.md#h31-every-turn-was-mined-five-times-and-the-fifth-pass-is-where-she-moved-a-hobby) for the measurements that set the pair, and why one threshold could not do the job.
...
- `memory.memory_extractor_max_window` *(int, `30`, min `2`)* — ceiling on how many **unmined** messages one extraction pass may take. The extractor keeps a per-session watermark in `kv_meta`, so this is now a backlog-drain cap rather than the size of every window: normally far fewer messages are unmined than this. A pass takes the *oldest* rows past the watermark, so a worker that fell behind drains from its front instead of stepping over the middle. See [H31](personality-backlog/health.md) for what the pre-watermark behaviour cost.
- `memory.memory_extractor_min_new` *(int, `4`, min `2`)* — floor of unmined messages below which a pass is skipped **without advancing the watermark**, so material accumulates. Matters because `compact_now` (the context-overflow squish) lowers the summariser's trigger to 2 messages: without a floor, the extractor would mine two rows at a time.
- `memory.memory_extractor_context_messages` *(int, `10`, min `0`)* — already-mined messages rendered *before* the new ones, under a header marking them read-only. Not zero by default because a claim's antecedent and the sentence carrying it often fall in different passes ("he finally finished it"). `0` shows only new material — cheaper, and occasionally unreadable.
...
- `memory.concept_hypothesis_interval_seconds` *(int, `1800`, min `60`)* — how often the idle worker looks for a belief worth asking about. Cheap: a `list_by(status="candidate")` read plus per-row gate probes, no LLM. Since H7 this is a heartbeat rather than a drafting rate: each run skips whichever pool (grounded / invented) already holds its half of `inventory_target`, so the shelf stops overflowing a spend rate of roughly one cue a day.
...
- `memory.concept_hypothesis_gap_min_importance` *(float, `0.55`, clamped `[0, 1]`)* — L32 importance a hunch must carry to be worth raising **out of a lull**. A bar the topic path deliberately does not have: on-topic, any open question is fair; out of silence, only one that matters justifies the weight. **Coarser than it looks (H7).** A grounded cue carries affect-lifted importance and lands anywhere (0.45–0.91 on the live shelf), but an *invented* cue has no grounded clusters to lift from, so its weight is exactly its kind's prior — and there are only thirteen of those. For that half of the shelf this is a kind whitelist wearing a float: every value in `(0.50, 0.60]` behaves identically, letting `conduct` and above open a lull while `ritual`, `taste`, `pursuit`, `narrative` and `communication_style` wait for a topical moment. `0.55` is the midpoint of that plateau, furthest from either boundary, so re-tuning a prior by a hundredth cannot silently flip a kind across it. Read the prior list in [`concept_kinds.py`](../app/core/concepts/concept_kinds.py) before moving it — any move smaller than one gap between priors does nothing.
...
- `memory.concept_core_openness_slots` *(int, `2`, min `0`)* — **openness reserve** on the always-on core lane. `core_lane_kinds()` returns only `core_always_on` kinds (`identity`, `value`, `boundary`, `generalization` — two anchors and two guides), so before this the lane was *structurally* incapable of pinning an aspiration, taste or pursuit no matter how the weights were tuned. This many slots are reserved for the strongest `generative`-role concepts, drawn from kinds otherwise ineligible for the lane. Cheap against a `context_budget_core_cap` of 15. An unfillable slot falls back to the ordinary lane, so the reserve never wastes a pin. `0` restores the guides-and-anchors-only lane exactly. The slots spread across *kinds* first and subjects second — with a flat draw the two `aspiration` subject buckets took every slot on the live graph and no other generative kind could be reached — and `tension` is excluded outright by its `pinnable = False`, since a standing friction nailed into every turn is the nagging L12's cooldown exists to prevent. It reaches T3 through the flex lane instead, where it has to earn the slot against the live turn (H10).
- `memory.concept_core_overfetch` *(float, `1.5`, clamped `[1.0, 4.0]`)* — how deep the always-on core lane draws relative to `context_budget_core_cap`, which is the **only** thing deciding how much of the pinned lane survives to the next turn. The lane keeps the `core_cap` most-rested of what it drew, so a pick carries forward only while the draw is shallower than the set habituation already marked stale — and what got marked is everything rendered last turn, *both* lanes (`core_cap + context_budget_concept_cap` rows). Carry-over is therefore positive iff `core_cap × multiplier < 2 × core_cap + concept_cap`, i.e. with the two caps equal, iff the multiplier is under `3`. This was a hard-coded `3` against two caps of `15` — a draw of 45 against 45 stamped, exactly on the edge — which is why the pinned lane shared *precisely* zero concepts with the previous turn across 1,160 measured turn pairs (P52). Measured at the live caps: `3.0` carries over 2% per turn and shows 41 distinct concepts across 8 turns; `2.0` carries 12%/30; **`1.5` settles at 53% (8 of 15 kept, 7 rotated) and 22 distinct**; `1.0` freezes the lane at 100%/15. So this trades breadth for continuity, and the default buys continuity — thirty facts a turn with nothing held over is not how attention works. Values at or above `3` stay reachable to restore the old behaviour; note they do not rotate "more thoroughly", they only lengthen the cycle.
...
- `tts.providers` *(object, `{}`)* — per-engine settings, keyed by provider name. Kept per-engine rather than global because neither field means the same thing across engines:
  ...
  - `loudness_target_dbfs` *(float or absent)* — per-engine override for `tts.loudness_target_dbfs`. Absent means "this engine's default", which is **not the same for every engine** and deliberately so: cloning engines take the global value, pocket-tts is off. Absent and `0.0` are therefore different — the first defers, the second insists. See the `tts.loudness_target_dbfs` row below for why they differ.
...
- `tts.loudness_target_dbfs` *(float, `-26.0`, clamped `[-40, -6]`; `0.0` → off)* — gated speech level every clip is matched to before affect gain. She speaks a turn a sentence at a time, each sentence is a separate synthesis, and no engine levels its own output: measured over one twelve-sentence turn, gated level varied by **8.3 dB on Chatterbox Nano and 8.4 dB on pocket-tts**, which sounds like her microphone moving between sentences. Normalising takes that to **0.0 dB** end to end. `-26` is where pocket-tts already averages, so this changes consistency and not loudness — and holds level constant across an engine swap too. It also makes `gain_db` work as intended: ambient compensation and `[[prosody:…]]` deltas were landing on a random base, so a tag asking for 3 dB softer was routinely swamped by an 8 dB swing the other way. Raise the target if she is too quiet against your room; set `0.0` to hear the engine raw for an A/B. Note it levels *between* sentences only — dynamics inside a sentence are untouched, since a limiter there would flatten her delivery. **Applies to cloning engines only; pocket-tts defaults to off** — the 8.4 dB looked like the same drift and it is not all drift. Nine sentences of known intended energy measured **+1.62 dB** between the high and low groups, so roughly a third of that spread is her delivery: the corrections it applied sorted by sentiment, pushing her three most excited lines down hardest and lifting the tired one. On Chatterbox the drift dominates that and consistency was the reported improvement; on pocket-tts it was reported as "she stopped being lively". Override either way per engine with `tts.providers.<name>.loudness_target_dbfs`. See [`app/audio/loudness.py`](../app/audio/loudness.py) and H48.
- `tts.timbre_match_limit_db` *(float, `4.0`, magnitude, capped `12.0`; `0.0` → off)* — ceiling on the per-clip brightness correction, in dB of shelf gain. Engines that clone from a reference clip shelve every sentence toward that clip's spectral tilt. This is the second half of the same problem as the row above, and it needed a measurement to separate them: with level pinned flat the remaining report was that her *warmth and level* still moved between sentences, and since level measured 0.00 dB gated and 0.23 dB K-weighted by then, that was brightness being heard as level. Regenerating **the same sentence** six times moved the low/high band ratio **4.2 dB** — identical words, so it is the model re-sampling timbre per call rather than her delivery. Matching takes a five-sentence turn from **5.79 dB to 0.69 dB** of spread. Because the target is her reference clip and not the engine's own average, it also closes Chatterbox's high-frequency deficit (mean tilt **+1.96 dB warmer than the reference → +0.15 dB**), which is the "a little muffled" impression. Bounded because content changes brightness legitimately — an excited three-word opener really is brighter than a long calm question — so the limit is set above the ~4 dB of noise and below the widest expressive swings. Only Chatterbox uses it; pocket-tts has no reference clip to aim at and is not audibly drifting. Set `0.0` for an A/B. See [`app/audio/timbre.py`](../app/audio/timbre.py).
- `tts.speech_rate_match_limit` *(float, `0.15`, magnitude, capped `0.25`; `0.0` → off)* — ceiling on the per-clip **tempo** correction, as a fraction. Third and last of the per-clip drifts, and the one that is easiest to misattribute: the report was "the first sentence was spoken really great, then the second one was much slower", which sounds exactly like a pacing setting misbehaving. It is not, and the check takes one look — `agent.tts_runtime_speed_enabled` is off by default, so every sentence in that reply was handed the *identical* rate multiplier. Synthesising **the same sentence** six times moves the delivered tempo by **10.7–20.9%**, and across every clip measured the spread was **24–28%**. Engines that can measure their reference's tempo stretch each sentence toward it, through the same WSOLA pass that already applies `speed`, so it costs nothing extra. Measured end to end on Chatterbox Nano over eight sentences: spread **24.1% → 5.9%**, worst clip **12.6% → 3.1%** off her pace, and **8 of 8** within 5% of it. It also removes a standing bias — Nano's raw output averages 6.26 syllables per second against her incumbent pocket-tts 6.55, so it had been running ~5% slow all along. `0.15` is the measured knee (`0.10` leaves the tails, `0.20` buys two more points of spread and flattens more); set `0.0` to hear the engine's own pacing. The target is derived from the `manifest.json` beside the reference clip, so a voice dropped in as a bare wav gets no correction rather than a guessed tempo. See [`app/audio/speech_rate.py`](../app/audio/speech_rate.py).
...
- `tts.pcm_pre_roll_ms` *(int, `600`, clamped `[250, 1500]`)* — how far ahead of the speaker the emit loop ships PCM, in milliseconds (sliced at 50 ms). The client still starts on the first slice; this only keeps more audio *queued* so a jittery hop (phone over Tailscale/Wi-Fi) does not punch holes in the sentence — heard as slow, hesitant speech, and as leftover tails landing on the next clip's start. 250 ms is the old localhost-only cushion. Raise toward 800–1000 if you still underrun on a bad link; barge-in cancel drops whatever was queued.
...
- `tools.web_search` *(bool, `true`)* — gates the `web_search` family: both the background workflow skill and the D3 synchronous brain-lane tool. The brain lane additionally requires `search.brain_tool_enabled`; the search backend (DuckDuckGo vs LangSearch) is configured separately under the `search` block below.
...
Web-search backend for every search path — the background workers (F1 fact-checker, G3 curiosity, F9 knowledge enrichment), the goal-workflow `web_search` lane, and the D3 synchronous brain-lane tool. `SessionController` builds **two** providers from this block: one shared by the background lanes, and one for the brain lane that drops the DuckDuckGo fallback and uses the shorter `brain_timeout_seconds` (a user is waiting on it). See [`app/llm/search/providers.py`](../app/llm/search/providers.py).
...
- `search.brain_tool_enabled` *(bool, `true`)* — **D3**: register the synchronous `web_search` tool on the conversational lane, so Aiko can look something up mid-turn and answer from it in the same reply. Costs roughly 7s to first word on the turns that actually search (measured: ~2.7s LangSearch + ~3s decision pass + ~1.5s to first token) and nothing on the rest — the P14 `web` gate family keeps it off banter turns. Turn this off to reclaim the latency while keeping the background lanes. Also requires `tools.web_search`. See [`docs/tools.md`](tools.md#synchronous-web-search-d3).
- `search.brain_timeout_seconds` *(float, `6.0`, floor `1.0`)* — request timeout for the brain lane only. Deliberately well under `timeout_seconds`: a background worker can afford to wait 12s, a user in mid-conversation cannot. The brain lane also never falls back to DuckDuckGo regardless of `fallback_to_duckduckgo`, since a slow HTML scrape in the fast lane is what got this tool removed from it originally.
...
- `agent.vision.in_turn_timeout_seconds` *(int, `25`, clamped `[5, 120]`)* — budget for the vision **calls**, once the GPU is ours. Much tighter than `timeout_seconds` because someone is watching: on expiry the pass is abandoned and the attachment falls back to the background workflow. The abandoned call is left to finish (Ollama has no cancel) so the model is warm for the next share — and it keeps its gate slot until it really ends, so a worker cannot start against a card that is still busy.
- `agent.vision.in_turn_wait_seconds` *(int, `180`, clamped `[0, 180]`)* — budget for **queueing**, before the call starts. The in-turn pass takes the worker LLM gate at `USER_BLOCKING`, ahead of every background worker, but the gate is non-preemptive, so a worker already mid-call has to finish. Separate from the call budget because the two failures want opposite answers: waiting out a background worker is normal and worth sitting through, while a slow *call* means the model is reloading under VRAM pressure and the turn should stop waiting. Sharing one budget between them is what produced "I can tell you attached an image but I can't see what's in it" — the queue ate all of it and the call never started. `0` means never wait (jump the queue or give up).

  The default is sized from what queues in front of it. The longest bounded background worker is the memory extractor (120s by its own timeout, measured at 106s on a real run), so a first guess of 45s gave up on an ordinary wait and produced that same reply. Both budgets are **shared across the images on one message and tracked apart** — queue time is not charged to the call budget, because billing a wait as if it were looking silently dropped the second photo of a two-image share.

  Waiting is genuinely the best option here, and it is worth knowing why before "fixing" it by letting the calls overlap. Measured against one model on one GPU: a 24-second generation became 52 seconds once a second request joined it, and that second request — 0.6s on its own — took 49. Ollama time-slices rather than running requests in parallel, so raising the gate's concurrency to get a photo answered sooner makes *both* callers slower. Splitting vision onto a separate model or device is the real way to remove the wait.
...
- `logging.ui_log_categories` *(list, `["ws", "channel", "settings", "voice", "audio"]`)* — allow-list of `source` values the endpoint accepts. Keeps a misbehaving client from spamming arbitrary lines. Entries outside it are dropped **silently**, so a missing token looks exactly like a client that never logged: `"audio"` was absent from `config/default.json` for the whole life of the iOS playback diagnostics (`contextDead`, `dropSuspended`, `firstPcm`, `clipStart`), which made every one of them unreachable while `rules/debugging.md` told you to grep for them.
