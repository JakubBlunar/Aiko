...
- `memory.context_budget_identity_cap` *(int, `2`, min `0`)* — **identity always-on lane**: up to this many active `kind="identity"` concepts (highest confidence first) are *pinned* into the region every turn regardless of turn relevance — this is how "who the user is / how Aiko wants to behave" reaches the prompt even when it doesn't match the topic. Pinned concepts bypass the concept `_cap` and `_min_relevance`, so they enrich on top of the relevance picks (they still consume the shared token budget). `0` disables the lane.
- `memory.context_budget_identity_min_confidence` *(float, `0.75`, clamped `[0, 1]`)* — confidence an identity concept must clear before it is eligible for the pinned lane, so only settled core traits are asserted every turn.
