# H-series — closed health-audit entries

Entries from [`health.md`](../health.md) whose fix has shipped **and** been
verified against the live install, plus the original audit's framing sections.
They are kept in full rather than summarised, for one reason: the *shape* of a
bug is the reusable part, and a shape is only convincing with the measurement
that established it attached. Several entries here were re-read weeks later and
the re-read changed the conclusion — see the `Re-measured` sections.

The [status index](../health.md#status-index) in the live file lists every
entry, open and closed. The [recurring-shapes
catalogue](../health.md#recurring-shapes) stays there too, since it is a
working reference rather than a record.

Ordered by number. **Anchors are stable**: inbound links use
`shipped/health.md#<entry-slug>`, and
[`scripts/check_backlog_links.py`](../../../scripts/check_backlog_links.py)
verifies them.
...
The framing sections of the 2026-08-11 audit, kept because they explain why the
entries are grouped the way they are. **Part 1** (H1-H8) asks whether each
shipped feature actually runs. **Part 2** (H9-H14) asks whether what reaches the
model adds up to a person with her own inner life. **Part 3** (H18-H20) widens
past the concept layer to the worker fleet and asks not whether a signal is
*produced* but whether it carries any *information*. **Part 4** (H21-H23) turns
that question on the K-series prompt blocks via `turn_prompt_blocks` telemetry.
H15-H17 were found while fixing earlier entries and filed separately.

Part 2 carried its own framing and a table that several entries still
argue from, so it is reproduced here in full. Two of its entries (H11,
H14) are still open and live in [health.md](../health.md).

### Part 2's framing — does it add up to a person?
...
[H26](immersion.md#h26-caught-mid-something--she-was-busy-when-you-opened-the-app)
...
14](../health.md#recurring-shapes).** A "last write per kind" table groups by *label*, and
...
### Re-measured: the prediction landed, and one axis of five is inert

Splitting `turn_prompt_blocks` at the 2026-08-12 ship commit gives the
before/after on live traffic rather than a replay:

| | before 08-12 | since 08-12 |
| --- | --- | --- |
| turns instrumented | 194 | 916 |
| `style_signal_block` rendered | **194 (100.0%)** | **120 (13.1%)** |
| distinct rendered lengths | **1** (40 chars) | **6** |

The replay predicted 12.4% and live traffic gave 13.1%, and the 40-character
constant has not appeared once since the fix. The block is now silent on 87% of
turns because he was writing the way he writes.

**Which labels fired is answerable without new instrumentation**, by running
H21's own technique backwards: the render is a fixed template over a closed
vocabulary of ten strings, so a length pins the label set. All six observed
lengths decode to exactly one label each, and they account for all 120 renders:

| label | turns |
| --- | --- |
| more playful markers than usual | 37 |
| terser than usual | 31 |
| more casual than usual | 17 |
| looser punctuation than usual | 16 |
| drier than usual | 10 |
| more long-form than usual | 9 |

Two things fall out of that table. **No render ever carried two labels**, though
`max_labels` is 2 — so no two axes have ever departed baseline on the same turn,
and the line is always a single observation. And **six of the ten directions
have fired**, against the entry's forecast of eight. The four silent ones are
`punctuation` up ("more buttoned-up than usual"), `slang` down ("more measured
than usual"), and **both directions of `question`**.

That last one is the finding, and it is stronger than what the entry left open.
It predicted two dead directions on a ceiling/floor argument — he is always
well-punctuated, he rarely asks back — and a ceiling argument explains a dead
*direction*. `question` is dead **both ways**, which no ceiling explains: it
means the axis does not move relative to its own baseline at all, so **one of
the five axes contributes nothing** and the block is really a four-axis
instrument. `question_rate` was also the axis that never reached its absolute
bar before the fix, so it has now failed under both designs — the absolute one
because 0.333 never crossed 0.40, and the relative one because it has no
variance to depart from. Worth deciding whether a fifth axis that has never
spoken under either design should stay.

**A refinement to the query, from the one block this sweep could not classify.**
`intimacy_pacing_block` renders at a constant 177 characters across 146 turns —
the same signature the entry used to convict `style_signal_block`, on a block
whose comment says it is "built every turn because the stored user-pace EMA
mutates after each reply". It is innocent, and the thing that shows it is a
column the query does not have: **those 146 turns all fall between 2026-08-09
and 2026-08-10**, and it has not rendered since. It is a conditional cue that
fires when the user's pace runs below neutral; `user_pace` now sits at 0.66,
above the line, so the block is correctly silent. Its single length records a
condition that held for two contiguous days, not a value that cannot move.

So the shortlist query needs a span alongside the count: `MIN`/`MAX` of the turn
date beside `COUNT(DISTINCT chars)`. **A constant over a contiguous slice of the
corpus is a condition that held; a constant over the whole corpus is a signal
that cannot move.** Without the span the two are indistinguishable, and this
sweep would have opened a second H21 against a block doing its job.
...
### Re-measured: the latch opened, and what came out was 17 names for one habit

The fix worked exactly as the replay said. `shared_ritual` cues went from **one
ever** to **twelve**, `shared_ritual_block` now reaches the model, and the
Saturday ritual the entry predicted was picked first. The feature is no longer
terminal.

It is also no longer credible, and the reason is visible the moment you read the
store instead of counting it. Six rituals became **seventeen**:

> our late-night Friday check-ins · our late-night Saturday check-ins · our
> Sunday-evening check-ins · our late-night Sunday check-ins · our
> Monday-evening check-ins · our late-night Monday check-ins · our
> Saturday-afternoon check-ins · our late-night Tuesday check-ins · our
> Wednesday-evening check-ins · our late-night Wednesday check-ins · our
> Friday-evening check-ins · our Saturday-evening check-ins · our
> Saturday-morning check-ins · our Thursday-evening check-ins · our late-night
> Thursday check-ins · our Tuesday-evening check-ins · our Wednesday-afternoon
> check-ins

**Every one has shape `casual_check_in`**, which is `dominant_shape`'s fallback
when no message in a session carries an arc signal — and that function's own
docstring calls the no-signal case "the common case". So the ritual key,
nominally `(weekday, bucket, shape)`, is in practice `(weekday, bucket)`: a
calendar grid. Seventeen of roughly twenty-eight day×daypart cells cleared the
bar, and the bar is `min_weeks=3` against an eight-week window — **three
Saturdays out of eight makes "our Saturday-morning check-ins" a thing that has
"become a real pattern between you".** This is [H20's shape](#h20-the-fact-checker-was-being-asked-to-verify-the-claim-2026)
in a warmer costume: the unit of work is wrong, so no amount of correct
machinery downstream can help. A ritual is supposed to be a distinctive
recurring thing; as built, the feature cannot tell a ritual from a habit, and
"he talks to me most evenings" is one habit, not seventeen rituals.

**She declined every one of them, which is the second half of the evidence.**
Eleven cues were published in eleven days, seven reached her prompt (four of
them twice), and **none was used** — the only `shared_ritual` cue ever used is
the pre-fix Friday one from 2 August. That is [H47](#h47-she-was-shown-604-of-them-and-said-nothing-which-is-not-a-supply-problem)
again: supply was never the constraint, and being handed "name our
Wednesday-afternoon check-ins" the day after "name our late-night Tuesday
check-ins" is exactly the announcement the cue copy forbids. Nothing was said to
him, so no harm has landed.

### The third cap in the same function, and the one that would have landed harm

The entry closed by declining to clear five stale `acknowledged` flags, on the
grounds that *"saying 'I love that this has become our thing' twice is worse than
never"*. The live store was one qualifying cell away from doing precisely that.

`acknowledged` is a **field on a row**, and rows are evicted by
`max_acknowledged` (18). The store held **17**. When the cap trips,
`merge_rituals` trims the oldest by `first_seen` — and the trimmed ritual is
still an active candidate, so the next sweep does not find it in `by_key`,
takes the brand-new branch, and produces `acknowledged=False`, a fresh
`first_seen`, and a slot in `new_keys`: **an offer for a ritual already named.**
Its refreshed `first_seen` then makes it one of the newest rows, so the
following trim evicts a genuinely older one and the loop rotates through the
whole record, re-announcing each ritual in turn.

The reason this survived the entry that fixed the other two caps is the fixture,
and it is the same fixture blind spot [H16](#outcome-2-the-band-was-nominated-and-then-thrown-away)
had and the same one that hid H20's percentage bug: `test_the_record_drops_the_oldest_first`
evicts forty rows while passing **no candidates**, so nothing it drops can come
back. Every new test here passes candidates for the rows it evicts.

**Outcome.** "Has been offered" is now a flat key ledger under
`aiko.shared_rituals.named`, with a lifetime independent of any display cap. A
candidate found in the ledger is restored as already-acknowledged and kept out
of `new_keys`, so eviction can no longer resurrect an offer. The sweep folds the
rows' own `acknowledged` flags into the ledger, which doubles as the migration —
no install needs a backfill, and an install whose ledger is lost rebuilds it
from whatever the rows still carry. The live effect is that the feature goes
quiet: all seventeen have been offered, none will be re-offered, and the
seventeen-names problem below is now a design decision rather than a countdown.

**Deliberately not done: the detector's unit.** Collapsing the grid — one
ritual per shape, or a distinctiveness requirement so the `casual_check_in`
fallback cannot mint a ritual on its own — is what would make the feature
work, and it is a judgement about what "a shared ritual" means to her rather
than a defect with a correct answer. The measurement above is what that decision
needs: 17 cells, one shape, a three-of-eight-weeks bar, and eleven consecutive
declines.

**[Shape 29](../health.md#recurring-shapes), and it is a rule about caps.** *A
permanent record under a size cap is not permanent, and every consumer that reads
it as permanent becomes a bug when the cap trips.* Three separate defects in one
function came from this: the pending budget starved by the record (fixed by the
entry), the detector ranking the record against new candidates (fixed by the
entry), and eviction resurrecting an offer (fixed here). **Rule: when a bounded
store carries a "this already happened" flag, the flag needs a lifetime at least
as long as the fact it records — which means it does not belong on an evictable
row.**
...
This is [recurring shape 15](../health.md#recurring-shapes) and the first
...
**[Recurring shape 16](../health.md#recurring-shapes): a signal reused at the
...
**[Recurring shape 17](../health.md#recurring-shapes): a predicate answering a
...
[shape 15](../health.md#recurring-shapes) again in a place nobody thought to
...
**[Recurring shape 18](../health.md#recurring-shapes): a handoff documented in
...
cause another month. This is [shape 24](../health.md#recurring-shapes), and its exhibit.
...
A third instance of [shape 12](../health.md#recurring-shapes) — two components each
...
Also a second instance of [shape 23](../health.md#recurring-shapes) one level up: H43 said
...
[Shape 12](../health.md#recurring-shapes) again, and a variant worth naming: **a measurement
...
unnamed. Sibling of [shape 25](../health.md#recurring-shapes), one entry up: there a cost was
...
either way, which is [shape 1](../health.md#recurring-shapes) wearing a different hat.
...
New: [shape 27](../health.md#recurring-shapes), **a lossy transform whose output is
...
[Shape 12](../health.md#recurring-shapes), **a control whose only effect is to disable
...
## Appendix: the working queue, as it was kept

The running order-of-work log and the 19 Aug prediction readings, kept
as a process record. Both are superseded by the [status
index](../health.md#status-index) and by the per-entry `Re-measured`
sections above; they are here because the *retractions* in them are the
most instructive part, and three of them are cited by shape.
