...
- [`web/src/features/chat/MessageBubble.reactions.test.tsx`](../../../web/src/features/chat/MessageBubble.reactions.test.tsx) — 12 source-level wiring tests: K31 badge strip wiring + fallback emoji, K32 reaction strip + hover tray gating on `canReact`, `onToggleReaction` dispatch, taxonomy contract assertion against the shared `types.ts` exports.
...
Pacing mirrors `WorldNoticeWorker`: quiet-gated by the scheduler, paced by its own cooldown, and it stands down while a garden visit is outstanding so it doesn't fight `GardenVisitWorker` over Aiko's location.
...
**Settings.** `agent.away_activities_enabled` (master) + `memory.away_activities_{interval,cooldown}_seconds` / `_min_gap_hours` / `_journal_max`, all clamped in `load_settings`. Persona guidance: the "Things I did while you were away" block in [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt). MCP debug ([`app/mcp/server.py`](../../../app/mcp/server.py)): `get_away_activities_state`, `force_away_activity(key)`, `force_away_activities_surface()` — repro is `force_away_activity()` → `force_away_activities_surface()` → `send_message(skip_tts=true)` → confirm the line in `get_last_response_detail.system_prompt`.

Tests: [`tests/test_idle_activity_worker.py`](../../../tests/test_idle_activity_worker.py) (activity pick + world mutation + journal ring + cooldown / garden-visit gates), [`tests/test_post_turn_away_activities.py`](../../../tests/test_post_turn_away_activities.py) (the arming gate matrix), and `AwayActivitiesProviderTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (slot ordering after turning_over + aggressive-mode retention).
...
**Distinct from** K27 day colour (categorical daily flavour, not a spendable resource), affect (fast objectless valence/arousal), and circadian (pure time-of-day, no memory of exertion). K68 is the slow, depletable, recovering one — the only one with a real feedback loop into avatar movement amplitude.

**Absorbs K39 (energy / spoons model), which is therefore closed.** The two entries were the same mechanic under different framings — a slow, spendable energy scalar that recovers over time and softens her register when low — and K39's own note asked for the two to be reconciled before either was built. K68 shipped the broader version (circadian baseline, per-turn spend *and* interest boost, feedback into avatar expressiveness and proactivity cadence), so nothing from K39 remains open.
...
The single most common companion failure is answering the *literal* message instead of the **need** behind it: reaching for problem-solving when the user just wants to be heard, or offering flat empathy when they actually want help thinking. K4 arc-tagging classifies the *topic* (`support` / `planning` / `playful`); `user_state` / `vocal_tone` read affect *magnitude* — but nothing classified the **response mode** the user is implicitly asking for. K69 is that missing classifier, and its whole value is **restraint**: the strongest beat is *not solving* when they didn't ask.

**Pure heuristic, no LLM on the hot path ([`implicit_need.py`](../../../app/core/conversation/implicit_need.py)).** `classify(user_text, *, arc, perceived_mood, perceived_energy, vocal_tags, min_confidence)` scores four modes — `witness` (vent → be heard, not fixed), `problem_solve` (explicit help-seeking → fixing IS the need), `reassure` (worry/anxiety → quiet the spin), `celebrate` (good news → match the high) — from three signal sources: weighted regex lexicons (the bulk; strong markers 2.0, soft cues 1.0), the live K14 affect read (low mood → witness/reassure, high mood → celebrate, anxious vocal tags → reassure), and the K4 arc as a weak `0.5` prior. The argmax above `min_confidence` (default 2.0) wins; everything below stays the silent `neutral`. **Tie-breaks favour restraint** (`witness > reassure > celebrate > problem_solve`), so an ambiguous emotional turn errs toward listening rather than fixing; a strong "how do I…" (2.0) still beats mild venting flavour, but a rhetorical "what do I do" (1.0) loses to real vent cues. Exclamation marks only *amplify* an existing celebrate hit, so "I'm so done!!!" never reads as celebration. The spec floated an LLM fallback on ambiguous turns; **deliberately not built** — the provider runs synchronously during prompt assembly, so a hot-path LLM call would tax every ambiguous turn; staying `neutral` and silent is the correct, cheap default. A background disambiguation pass is a possible fast-follow.

**Provider ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py) `_render_implicit_need_block`).** Provider-time (not a post-turn stash) so the steer lands on the SAME turn that's about to reply — answering the need *is* this reply. Gathers the three corroborating signals cheaply: arc from `_arc_store.get_or_default` (lags one turn, weak prior), a *same-turn* `_user_state_estimator.estimate(user_text)` for live perceived mood/energy (the J9 pattern), and `_last_vocal_tone` tags under lock. Stashes `_last_implicit_need` for MCP; `_implicit_need_force_mode` is a one-shot debug bypass. Best-effort throughout — any failure returns `""`.

**Tiering.** `implicit_need_block` lands in **T6 detectors, right after `misattunement_block`** (both are live-read "how to respond to THIS message" steers). **NOT dropped under `aggressive=True`** (sibling of misattunement — answering the need instead of the literal words matters most on a tight budget) and **NOT in the K16 grounding-line suppression matrix** (the fused line carries no need signal).

**Settings.** `agent.implicit_need_enabled` (master) + `memory.implicit_need_min_confidence` (score floor, default 2.0, clamp ≥0.5). **Persona**: a new bullet in the "Reading {user_name}" block of [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt) teaches the four steers (witness → resist solving; problem_solve → get concrete but lead with empathy; reassure → slow the spin first; celebrate → let the win land, no "but") and the default-to-the-ear rule when unsure. **MCP** ([`self_state_tools.py`](../../../app/mcp/server_tools/self_state_tools.py)): `get_implicit_need_state` (switch, floor, last verdict with per-mode scores + reasons, force flag) and `force_implicit_need(mode)` (one-shot pin → next provider call renders that mode's steer). Grep `implicit-need: mode=…` on the `app.implicit_need` logger.

**Distinct from** K4 (topic type, not response mode), K23 misattunement (post-hoc "you misread him" after a short/pivot reply), and K8 rupture (post-hoc affect drop). K69 is the *forward* read on the live message — what kind of reply it's asking for.

**Tests:** [`tests/test_implicit_need.py`](../../../tests/test_implicit_need.py) (25 pure: each mode, the restraint tie-break, arc-as-weak-prior, exclamation-only-amplifies, affect/vocal corroboration, min-confidence floor, neutral defaults, render), `ImplicitNeedProviderSlotTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (lands, silent-when-empty, **retained under aggressive**, exception swallow, T6-after-misattunement tier registration), `test_implicit_need_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).
...
**Data source — reuse, don't re-sample.** K70 reads the H3 mood-drift daily ring (`aiko.mood_drift_samples` — `DriftSample{date, valence, closeness, humor, trust, comfort}`, capped at 21 days, populated by the H3 `MoodDriftSampleWorker`). Zero new per-day sampling cost; with H3 sampling disabled K70 silently no-ops.

**Pure detector ([`growth_witness.py`](../../../app/core/relationship/growth_witness.py)).** `detect_growth(samples, *, min_samples, min_valence_delta, min_axis_delta, detail)` averages the **oldest third** of the ring against the **newest third** and returns the strongest **positive** shift above its threshold: `lighter` (valence rose ≥ `0.25`), `comfort` (comfort axis rose ≥ `0.30`), or `open` (trust axis rose ≥ `0.30`). Largest delta wins; mood breaks ties. Needs `min_samples=10` (a real multi-week history). **Positive-only by design** — a durable *downturn* stays with H3 `sustained_low` / a wellbeing track; K70 never tells the user he's gotten worse. `render_inner_life_block` frames the span honestly ("these past couple of weeks" only when the ring spans ≥ 12 days, else "lately") and weaves in an optional corroborating `detail`.

**Silent producer ([`growth_witness_worker.py`](../../../app/core/proactive/growth_witness_worker.py)).** `GrowthWitnessWorker` is an `IdleWorker` (~6h cadence) gated by a **14-day wall-clock cooldown** (`growth_witness.last_fired_at`) and a **finding-signature** gate (`growth_witness.last_signature`, kind + magnitude bucket) so the identical "you seem lighter" never re-drafts back-to-back. On a clearing finding it appends one `{at, kind, magnitude, span_days, signature, detail}` entry to the `aiko.growth_witness` kv ring. Corroboration: reads the optional goal store for an active goal with a `last_progress_note` → "the work he's been putting into {goal}" woven into the cue (never a standalone trigger). Logs `growth-witness drafted: kind=… mag=… span=…d`.

**Consumer ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py) `_render_growth_witness_block`).** Watermark-gated one-shot (`growth_witness.last_surfaced_at`), **independent of the gap-return cue family** (like `follow_up`): a durable longitudinal observation is rare enough that surfacing on the next turn after drafting is fine, and the cue copy tells Aiko to wait for a warm moment. The watermark advances only after the line actually renders. Logs `growth-witness fire: at=… kind=…`.

**Tiering.** `growth_witness_block` lands in **T6, right after `follow_up_block`** (the watermark-gated cue-producer family), **retained under `aggressive`** (the invocation is unconditional — a watermark-consuming one-shot must not be silently dropped after it ran, or the cue is lost), and **NOT in the K16 grounding-line suppression set**.

**Distinct from** H3 mood-drift (shorter arc / lower bar / present-tense "you've seemed off lately" + bidirectional), the axes block (narrates the *relationship* crossing thresholds, not the user growing), and H1 mood-drift narration of *her* mood. K70 alone is the long-arc, positive, "you as a person have grown" beat.

**Settings.** `agent.growth_witness_enabled` / `_check_interval_seconds` (default 21600, floor 60) / `_cooldown_days` (default 14) + `memory.growth_witness_min_samples` (10, floor 2) / `_min_valence_delta` (0.25) / `_min_axis_delta` (0.30) / `_journal_max` (4, floor 1). **Persona**: a "Notice him across time" bullet in the "Reading {user_name}" block of [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt) — say it once, gently, only on an open warm moment, as something *felt watching him*, never a status report or fished compliment. **MCP** ([`proactive_task_tools.py`](../../../app/mcp/server_tools/proactive_task_tools.py)): `get_growth_witness_state` (switch, cadence/cooldown, ring, pacing + surfacing watermarks, **dry-run** detection over the live ring), `force_growth_witness_draft` (arms the gate bypass + runs once), `force_growth_witness_surface` (one-shot watermark bypass). Repro: `force_growth_witness_draft()` → `force_growth_witness_surface()` → `send_message(skip_tts=true)` → confirm the "Something you've quietly noticed …" line in `get_last_response_detail.system_prompt`. Grep `growth-witness` on the `app.growth_witness` / `app.growth_witness_worker` loggers.

**Tests:** [`tests/test_growth_witness.py`](../../../tests/test_growth_witness.py) (durable valence/comfort/trust rises, too-few-samples / flat / **downturn-never-fires** / sub-threshold silence, mood-wins-tie, signature bucketing, detail passthrough, render span framing + detail weave + unknown-kind, ring round-trip/cap/garbage), `GrowthWitnessProviderSlotTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (lands, silent-when-empty, **retained under aggressive**, exception swallow, T6-after-follow_up registration), `test_growth_witness_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).

---

## K71. Self-callback — her own continuity over time

The symmetric self-side of K63: where K63 reaches back to something *the user* said weeks ago, K71 lets Aiko reference **her own** aged states and close the loop on them ("a while back I told you I'd been feeling restless — that's eased off now", "I said I wanted to get back into astronomy, and I actually did"). A companion with real continuity remembers what *she* was going through, not just what you were.

**Pure module ([`self_callback.py`](../../../app/core/affect/self_callback.py)).** `classify_self_memory(content)` buckets one of Aiko's own `self` / `reflection` rows into `feeling` (a past emotional state — narrow first-person regex over restless/anxious/low/tired/… so biographical facts never match), `intention` (a voiced want — "I want to / I've been meaning to / I hope to / …"), or `other` (skipped). Feeling beats intention on a row with both. `select_candidate(memories, *, now, min_age_days, exclude_signatures)` filters to rows ≥ `min_age_days` old, classifies, drops `other` + excluded signatures, and returns the **oldest** qualifier (reads most like "a while back"). `render_inner_life_block` produces the private cue and **leaves the resolution read to the model** — it has Aiko's current affect / day-colour in context, so the cue just says "if it eased, say so; if you followed through, own it; if it's still true or slipped, admit that" rather than computing resolution in code.

**Silent producer ([`self_callback_worker.py`](../../../app/core/proactive/self_callback_worker.py)).** `SelfCallbackWorker` is an `IdleWorker` (~6h cadence) gated by a **10-day wall-clock cooldown** (`self_callback.last_fired_at`); per-memory de-dup is structural (each ring entry's signature is `self:<memory_id>`, and the picker excludes the recent ring signatures, so the same memory never re-drafts). Reads her own memories via `MemoryStore.iter_by_kinds(("self", "reflection"))` (one locked mirror walk). Appends `{at, memory_id, kind, excerpt, age_days, signature, source}` to `aiko.self_callback`. Logs `self-callback drafted: id=… kind=… age=…d source=…`.

**Robust selection — worker-model pass with heuristic fallback (K71 follow-up).** Regex feeling/intention classification is the brittle link: it misses paraphrases ("things have felt heavy") and false-positives on biographical facts. Because the worker runs **at most ~monthly** (the 10-day cooldown is the natural budget), an LLM selection pass costs essentially nothing, so it's the default. Flow: the cheap heuristic acts as a **prefilter** — `gather_aged_candidates(memories, …)` returns up to 12 aged rows (keeping `other`-classified rows the regex would drop, so the model can catch missed feelings *and* reject facts), oldest-first; `build_selection_prompt` + the worker-model `chat_stream(format_json=True, surface="self_callback")` ask for a single `{"memory_id", "kind", "worth"}` JSON pick; `parse_selection(raw, valid_ids)` validates the id is in range, the kind is feeling/intention, and `worth != false`. **Any** failure (no worker client, LLM off, `worth=false`, unparseable, out-of-range, exception) falls through to the original pure-Python `select_candidate` — the upgrade only ever *adds* robustness, never regresses. The ring records `source = "llm"|"heuristic"` so a debugger can see which path drafted. Mirrors the `BeliefInferenceWorker` LLM-idle-worker pattern (worker client = `_maintenance_client`, model = `_effective_worker_model`, shared `_fact_check_cancel`).

**Consumer ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py) `_render_self_callback_block`).** Watermark-gated one-shot (`self_callback.last_surfaced_at`), independent of the gap-return cue family. **T6, right after `growth_witness_block`** (the watermark-gated cue-producer family — her continuity sits beside his growth), retained under aggressive (unconditional invocation; a watermark-consuming one-shot must not be silently dropped after it ran).

**Distinct from** K28 turning-over (her *current* preoccupation, surfaced on gap-return from *recent* 24-72h reflections — K71 revisits an *aged* ≥2-week past self-state) and K63 (user callbacks, not her own).

**Settings.** `agent.self_callback_enabled` / `_check_interval_seconds` (21600, floor 60) / `_cooldown_days` (10) / `_llm_enabled` (true — kill-switch back to the pure heuristic) + `memory.self_callback_min_age_days` (14, floor 1) / `_journal_max` (4, floor 1). **Persona**: a "Closing the loop on myself (my own continuity)" block in [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt) teaching the honest "huh, that's changed" delivery — never a progress report, admit it if a thread didn't resolve. **MCP** ([`proactive_task_tools.py`](../../../app/mcp/server_tools/proactive_task_tools.py)): `get_self_callback_state` (switch, cadence/cooldown, ring, watermarks), `force_self_callback_draft` (cooldown bypass + run once), `force_self_callback_surface` (watermark bypass). Repro: insert a `self` memory with `created_at` 30 days ago and feeling/intention content → `force_self_callback_draft()` → `force_self_callback_surface()` → `send_message(skip_tts=true)` → confirm the cue in `get_last_response_detail.system_prompt`. Grep `self-callback` on the `app.self_callback` / `app.self_callback_worker` loggers.

**Tests:** [`tests/test_self_callback.py`](../../../tests/test_self_callback.py) (classify feeling/intention/fact, feeling-beats-intention, select aged + oldest-wins + too-young / excluded / no-qualifier, excerpt truncation, render both kinds + empty/unknown silence, ring round-trip/cap/signatures/garbage, plus gather-keeps-other / age+exclude+cap, build-prompt, parse-selection valid/worth-false/out-of-range/bad-kind/garbage), [`tests/test_self_callback_worker.py`](../../../tests/test_self_callback_worker.py) (LLM pick used, worth-false / exception / no-client / disabled-provider all fall back to heuristic, ring records source), `SelfCallbackProviderSlotTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (lands, silent-when-empty, retained-under-aggressive, exception swallow, T6-after-growth_witness registration), `test_self_callback_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).

---

## K72. Wellbeing concern — gentle care, never a nag

The session clock (K-time4) notices a long sitting *neutrally*; H3 mood-drift narrates a low *mood* stretch. Neither turns a multi-day **behavioral** pattern of self-neglect into bounded, genuine *care*. A real partner notices when you've been online at 3am several nights running, when you keep mentioning you haven't slept or eaten, when the weight's sat heavy for days — and says something soft, *once*, because they care. K72 is the worried sibling of K70 (which is positive-only): a rare detector over slow signals that, only when a real worrying pattern clears a high bar, arms one gentle "you doing okay?" cue. The whole risk is becoming a nag / a health app, so it's gated *hard*: a long worker cooldown, one concern per pattern (date-free signature), the cue copy forbids lecturing + repeating, and the persona drops it the instant the user deflects.

**Pure module ([`wellbeing_concern.py`](../../../app/core/relationship/wellbeing_concern.py)).** Three independent detectors over multi-day aggregates the worker feeds it: `detect_late_nights(late_night_dates, min_nights)` (distinct small-hours days, `late_nights:N` signature so an *escalation* re-fires), `detect_self_neglect(neglect_days, categories, min_days)` (explicit "haven't slept / eaten" mentions across ≥2 days; `classify_neglect_text` is a curated negation-aware regex set — `haven't slept` with a negative-lookahead so "slept this well" never matches; food + sleep categories), `detect_rough_stretch(samples, min_run, threshold)` (a trailing run over the H3 `DriftSample` ring, gated *deeper + longer* than H3 sustained_low — 5 days ≤ −0.25 vs H3's 3 ≤ −0.15 — so K72 reads as concern, not mood-narration). `pick_concern(...)` runs them in fixed priority (self-neglect → late nights → rough stretch; most concrete wins) and returns **at most one** `ConcernFinding` so a worried turn never stacks three worries. `render_inner_life_block` produces one private cue with a baked-in example line and the "check in ONCE, never a lecture / nag / diagnosis, drop on deflect" discipline — NEVER spoken verbatim.

**Silent producer ([`wellbeing_concern_worker.py`](../../../app/core/proactive/wellbeing_concern_worker.py)).** `WellbeingConcernWorker` is an `IdleWorker` (~6h cadence) gated by a **7-day wall-clock cooldown** (`wellbeing_concern.last_fired_at`) + a date-free signature gate (`wellbeing_concern.last_signature`, so the same "a few late nights" never re-drafts but a worse count breaks through). One bounded range scan over recent user `messages` (timestamps for small-hours activity, content for the lexical scan), local-tz the same way K3 `ScheduleLearner` does (`dt.astimezone()`), plus the H3 mood-drift ring for the rough-stretch fallback. Appends `{at, kind, detail, severity, signature}` to `aiko.wellbeing_concern`. All best-effort: a SELECT failure yields no finding, never a crashed tick. Logs `wellbeing-concern drafted: kind=… detail=… severity=…`.

**Consumer ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py) `_render_wellbeing_concern_block`).** Watermark-gated one-shot (`wellbeing_concern.last_surfaced_at`), independent of the gap-return cue family. **T6, right after `self_callback_block`** (the watermark-gated cue-producer family), retained under aggressive (a watermark-consuming one-shot must not be silently dropped after it ran). **NOT** in the K16 grounding-line suppression set — the grounding line carries no multi-day wellbeing signal.

**Distinct from** K23 misattunement (per-turn short-reply detection) and K14 engagement (per-turn) — both read a single turn; K72 only reads slow, multi-day signal. Distinct from H3 sustained_low (mood *reflection* at a lower bar) — K72's rough-stretch is a deeper/longer low surfaced as an *active care check-in*, and behavioral signals (late nights / self-neglect) outrank it.

**Open follow-up.** Late-nights currently infer "he was up" from chat-message timestamps. C6's activity sessions are a better sensor; feed them into these detectors rather than minting a companion cue — [C9](../proactive.md#c9-activity-duration-as-wellbeing-evidence-k72).

**Settings.** `agent.wellbeing_concern_enabled` / `_check_interval_seconds` (21600, floor 60) / `_cooldown_days` (7) + `memory.wellbeing_concern_window_days` (7) / `_late_night_min` (3) / `_neglect_min_days` (2) / `_rough_run` (5) / `_rough_threshold` (−0.25) / `_journal_max` (4); the small-hours window (01–05h) is a module constant. **Persona**: a "When I'm worried about you" block in [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt) teaching the one-soft-check-in-then-drop delivery and explicitly forbidding sleep-hygiene lectures / moralising / asking twice. **MCP** ([`proactive_task_tools.py`](../../../app/mcp/server_tools/proactive_task_tools.py)): `get_wellbeing_concern_state` (switch, cadence/cooldown, ring, watermarks, **dry-run** re-detection over the live signal), `force_wellbeing_concern_draft` (cooldown+signature bypass + run once), `force_wellbeing_concern_surface` (watermark bypass). Repro: seed 3+ user messages at local 3am (or "haven't eaten" on 2 days) → `force_wellbeing_concern_draft()` → `force_wellbeing_concern_surface()` → `send_message(skip_tts=true)` → confirm the "Something you've quietly clocked …" line in `get_last_response_detail.system_prompt`. Grep `wellbeing-concern` on the `app.wellbeing_concern` / `app.wellbeing_concern_worker` loggers.

**Tests:** [`tests/test_wellbeing_concern.py`](../../../tests/test_wellbeing_concern.py) (classify sleep/food negations + positive-not-flagged + both, detect late-nights threshold/dedupe/escalation, detect self-neglect days+category/detail/below-min, detect rough-stretch run/break/deep-bar, pick_concern priority + nothing, render per-kind + unknown-empty + never-lecture-language, ring round-trip/cap/garbage), [`tests/test_wellbeing_concern_worker.py`](../../../tests/test_wellbeing_concern_worker.py) (drafts late-nights / self-neglect-outranks / rough-from-ring, no-finding clean, cooldown blocks + force bypass, same-signature suppressed, disabled, SELECT-failure silent), `WellbeingConcernProviderSlotTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (lands, silent-when-empty, retained-under-aggressive, exception swallow, T6-after-self_callback registration), `test_wellbeing_concern_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).

---

## K73. Shared ritual formation — "this is becoming our thing"

K3 (`ScheduleLearner`) detects the *user's solo* recurring slots ("gym Tuesdays") and silently populates a profile field. What it can't see is the **dyadic** ritual — the patterns in how *the two of them* interact: a Friday-evening check-in that's quietly become a standing date, a recurring late-night heart-to-heart, a Sunday-morning catch-up. Naming an emergent shared tradition ("I kind of love that this has become our Friday thing") is one of the warmest long-relationship beats there is. K73 mines conversation **timing + arc/topic *shape* recurrence between the two** for a `(cadence, shape)` pattern that has genuinely repeated, surfaces it **once** as a warm acknowledgment, then lets it become a light standing reference. Distinct from K3 (user-only timing routine, passive profile field) and anniversaries (one-off milestone *dates*). K73 is the *shape*-aware, dyadic, actively-named sibling.

**The "shape" problem.** The `conversation_arc` table only stores the *current* arc (no timestamped history), so K73 can't query "what arc was each past Friday?". Instead the worker derives the shape itself: a new pure `conversation_arc.estimate_arc(text)` helper (shares the exact `_ARC_PATTERNS` regex ladder `ArcEstimator` uses, no store/confidence bookkeeping) tags each user message's coarse arc; the modal non-empty arc of a `(weekday, bucket, ISO-week)` session is that session's *shape* (no signal at all → `casual_check_in`, itself a perfectly good "catch-up" ritual shape).

**Pure module ([`shared_ritual.py`](../../../app/core/relationship/shared_ritual.py)).** `dominant_shape(arcs)` (modal arc, else default). `detect_rituals(slot_weeks, *, total_weeks, min_weeks, min_share, max_rituals)` keeps `(weekday, bucket, shape)` slots that lit up in **both** ≥ `min_weeks` distinct ISO weeks *and* ≥ `min_share` of the window's weeks — recurrence, not raw volume, is what makes a ritual. Naming is deterministic: `_SHAPE_LABELS` (arc → noun: `casual_check_in`→"check-ins", `support`→"heart-to-hearts", `reflection`→"wind-downs", `planning`→"planning sessions", `playful`→"banter", `silly`→"goofing-around sessions") × `_BUCKET_ADJ`/`_BUCKET_DISPLAY` (e.g. `("friday","evening")`→"Friday-evening" / "Friday evenings", `("saturday","late")`→"late-night Saturday") build the full `label` "our Friday-evening wind-downs". `merge_rituals(existing, candidates, *, now_date, max_active)` folds candidates into the persisted list: an existing ritual still a candidate is **updated** but keeps its `acknowledged` flag + `first_seen`; an **acknowledged** ritual that fell out of the candidate set is **kept** (it became a real thing — permanent record); an **un-acknowledged** one that fell out is **dropped** (never announced, no harm). `render_inner_life_block(ritual, *, user_display_name)` produces one warm, optional "name it ONCE, then let it be a light standing reference" cue — NEVER spoken verbatim. kv helpers over `aiko.shared_rituals`.

**Silent producer ([`shared_ritual_worker.py`](../../../app/core/proactive/shared_ritual_worker.py)).** `SharedRitualWorker` is an `IdleWorker` (~daily cadence). One bounded range scan over recent user `messages`, local-tz the same way K3 does (reuses `_HOUR_TO_BUCKET` / `_weekday_name_local` / `_iso_week_key`), groups by `(weekday, bucket, ISO-week)` session → `dominant_shape` per session → `slot_weeks`, then `detect_rituals` + `merge_rituals` + persist. **Idempotent by construction** (it just keeps the store fresh), so it needs no wall-clock cooldown of its own — the *surfacing* cadence lives in the provider. A `min_messages` floor gates any naming on a fresh install; `force_next()` bypasses it for testing. All best-effort. Logs `shared-ritual sweep: messages=… candidates=… new=…`.

**Consumer ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py) `_render_shared_ritual_block`).** Claims the pending `shared_ritual` cue from the pool (`take_pool_cue`); spacing is the type's `CuePolicy.surface_cooldown_hours` (three days) so a burst of newly-qualified rituals can't announce back-to-back. **T6, right after `wellbeing_concern_block`** (the cue-producer family), retained under aggressive. **NOT** in the K16 grounding-line suppression set. *(Before the cue-pool migration this provider picked the strongest un-acknowledged ritual itself and set `acknowledged` on fire; the flag is now set by the worker at publish time. Rituals in the live store flagged before 2026-08 come from that older path.)*

**The two budgets are independent, and that is load-bearing.** `max_active` caps *pending* rituals; acknowledged ones are a permanent record with their own `max_acknowledged` cap (18, oldest `first_seen` dropped first). A single shared cap is what the first version had, and it made the feature terminal: `keep_pending = max(0, max_active - len(ack))` hits zero the moment six rituals are acknowledged, after which every newly-formed ritual was created, reported in `new_keys`, then trimmed away before the save — so it was "new" again next sweep, forever — and `pick_unacknowledged` returned `None` on all eight recorded runs. The worker also asks `detect_rituals` for `max_active + len(acknowledged)` candidates, or the record crowds new patterns out one stage earlier. `new_keys` is computed **after** the trim so the sweep log cannot report a row that was discarded. See health.md H22.

**Surface.** `get_together_summary` ([`world_mixin.py`](../../../app/core/session/world_mixin.py)) carries a read-only `shared_rituals` list; the **Together tab** ([`TogetherTab.tsx`](../../../web/src/features/settings/TogetherTab.tsx)) renders an "Our things" section (♥ for acknowledged, `Nw` recurrence badge). `SharedRitual` added to [`types.ts`](../../../web/src/types.ts).

**Settings.** `agent.shared_ritual_enabled` (true) / `_check_interval_seconds` (86400, floor 60) / `_surface_cooldown_days` (3) + `memory.shared_ritual_window_days` (56) / `_min_weeks` (3) / `_min_share` (0.34) / `_max_active` (6, *pending* rituals) / `_min_messages` (30). **Persona**: an "Our things (shared rituals)" block in [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt) teaching the name-it-once-then-light-reference cadence (naming it twice as A Big Thing kills it; absence of a cue is normal). **MCP** ([`proactive_task_tools.py`](../../../app/mcp/server_tools/proactive_task_tools.py)): `get_shared_ritual_state` (switches, thresholds, full store, strongest pending, watermark), `force_shared_ritual_draft` (min-messages bypass + run once), `force_shared_ritual_surface` (cooldown+ack bypass). Repro: seed user messages on the same weekday/hour across 3+ ISO weeks → `force_shared_ritual_draft()` → `force_shared_ritual_surface()` → `send_message(skip_tts=true)` → confirm the "... have become our thing" line in `get_last_response_detail.system_prompt`. Grep `shared-ritual` on the `app.shared_ritual_worker` / `app.session` loggers.

**Tests:** [`tests/test_shared_ritual.py`](../../../tests/test_shared_ritual.py) (dominant-shape default/modal/single, detect threshold/share/labels/sort+cap, merge add/preserve-ack/drop-pending/keep-ack, `BudgetStarvationTests` for the H22 latch — a new ritual survives a full record *and is offerable*, `new_keys` names only survivors, the record stays bounded and drops oldest-first, pending still capped — pick strongest/none, render warm-cue/empty/mark, kv round-trip/garbage), [`tests/test_shared_ritual_worker.py`](../../../tests/test_shared_ritual_worker.py) (names Friday-evening check-ins, support shape, min-messages floor + force bypass, two-weeks-no-ritual, disabled, acknowledged-preserved-across-runs), `SharedRitualProviderSlotTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (lands, silent-when-empty, retained-under-aggressive, exception swallow, T6-after-wellbeing registration), `test_shared_ritual_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).

---

## K75. User-expertise calibration — match explanation depth

**Motivation.** K66 (earned familiarity) reads how deep the *shared history* on a topic is; K75 is the orthogonal read of the **user's own competence** on it. Over-explaining to a senior dev ("a variable stores a value…") is as relationship-damaging as under-scaffolding a novice — both say "I'm not actually tracking who you are." K75 keeps a light per-topic-cluster competence estimate (`novice` / `familiar` / `expert`), learned from the user's *own language* in that cluster, and renders a one-line depth steer so Aiko pitches explanation at the right level. Distinct from K66 (history depth *between* them), F10i topic_confidence (how much *Aiko* knows), and K25 (confidence decay).

**Pure module** ([`app/core/conversation/user_expertise.py`](../../../app/core/conversation/user_expertise.py)). `classify_message(text)` returns a signed signal in `[-1, +1]` (novice ↔ expert) from **regex only** (no embeddings) — expert-leaning: correction/precision markers ("actually…", "that's not right"), expert self-report ("I built…", "I'm an engineer", "in my experience"), jargon/code shape (backticks, snake_case/camelCase, `foo()`, CLI flags, acronyms, long words); novice-leaning: novice self-report ("I'm new to…", "never used…", "don't really know…"), basic questions ("how do I…", "can you explain", "eli5", "what's a…"). Neutral chit-chat → `None` (leaves the estimate alone). `update_state` folds a signal into a per-cluster EMA (`learning_rate=0.25`, converges over ~4-8 messages); `band_for` bands the running score (`expert ≥ +0.35`, `novice ≤ −0.35`, `familiar` middle, `None` below `min_samples=4`); `render_block` renders the depth steer for the two **confident** bands only (`familiar`/`None` → silent). State is a `cluster_id → ExpertiseState{score,samples,updated_at}` map in `kv_meta` under `aiko.user_expertise`.

**Learned half** — post-turn in [`post_turn_mixin.py`](../../../app/core/session/post_turn_mixin.py) (beside J12 intimacy pacing). **Classify-first**: `classify_message(user_text)` runs before any embed, so a neutral turn pays *nothing* and never drags the estimate; only a signal-bearing turn embeds `user_text`, resolves the live cluster via `TopicGraph.best_clusters_for(min_sim=0.45)`, and folds the signal into that cluster's EMA. The cluster min-sim gate doubles as the noise filter (casual "what's up?" doesn't match a topic cluster, so it's never learned). Logs `user-expertise learn: cluster=… signal=… score=… samples=…`.

**Consumer** — `_render_user_expertise_block(user_text)` ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py)), same cheap cluster-match shape as K66: master switch → `len≥8` → persistent graph + embedder → cooldown/force → embed → `best_clusters_for` → read the kv state for the matched cluster → `band_for` → render the steer for `expert`/`novice` only. Cooldown-gated (`user_expertise_cooldown_turns=12`) to stay rare. Registered as `user_expertise_block` in **T6** right after `earned_familiarity_block` (both topic-graph register cues); dropped under `aggressive=True` (non-critical). Logs `user-expertise fire: cluster=… band=… label=…`.

**Settings.** `agent.user_expertise_enabled` (true) + `memory.user_expertise_min_sim` (0.45) / `_learning_rate` (0.25) / `_min_samples` (4) / `_novice_threshold` (−0.35) / `_expert_threshold` (0.35) / `_cooldown_turns` (12). **Persona**: a "Meeting {user_name} at his level (explanation depth)" block in [`aiko_companion.txt`](../../../data/persona/aiko_companion.txt) teaching peer-to-peer vs scaffold-gently and forbidding announcing the read out loud. **MCP** ([`memory_worker_tools.py`](../../../app/mcp/server_tools/memory_worker_tools.py)): `get_user_expertise_state` (switch, knobs, cooldown, last fire, full learned per-cluster map with bands + labels), `force_user_expertise_surface` (cooldown + min-sim + min-samples=1 bypass). Repro: send a few on-topic expert/novice messages → `get_user_expertise_state` to confirm the cluster's band → `force_user_expertise_surface()` → `send_message(skip_tts=true)` → confirm the "Depth check: …" line in `get_last_response_detail.system_prompt`. Grep `user-expertise` on the `app.session` logger.

**Tests:** [`tests/test_user_expertise.py`](../../../tests/test_user_expertise.py) (classify short/neutral→None + expert/correction/jargon positive + novice/basic-question negative + clamp, update EMA first/blend/clamp, band None-below-min-samples/expert/novice/familiar, render expert/novice/familiar-empty, kv round-trip/garbage, provider expert/novice surface + familiar/insufficient/no-match/disabled/short blank + cooldown + force bypass), `UserExpertiseProviderSlotTests` in [`tests/test_prompt_assembler.py`](../../../tests/test_prompt_assembler.py) (lands, silent-when-empty, dropped-under-aggressive, exception swallow, T6-after-earned_familiarity registration), `test_user_expertise_settings_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).

---

## K76. Affective memory salience — flashbulb encoding

Human memory isn't flat: moments that hit you *emotionally* burn in harder and fade slower (a flashbulb memory). Aiko's memories carried a salience + tiered decay, but salience at **write-time** ignored how she felt when the memory formed — a fact learned during a K8 rupture or a K57 strong-emotion episode was encoded with the same weight as small talk. K76 fixes that with a cheap encoding-time mechanic: **no worker, no LLM, no schema change.**

**Pure module ([`flashbulb.py`](../../../app/core/memory/flashbulb.py)).** `compute_charge(arousal, episode_intensity, …)` folds two live signals into a `[0, 1]` emotional charge: arousal contributes only its activation *above* `arousal_neutral` (AffectState arousal rests at ~0.4, so only genuine activation counts), rescaled to [0,1] and weighted; the active K57 episode intensity contributes directly, weighted; the sum is clamped. `apply_flashbulb(base_salience, …) -> FlashbulbResult{salience, charge, boost}` returns `salience = clamp(base + max_boost * charge)`. The elegant part: **neutral affect → 0 charge → 0 boost**, so low-arousal small talk is untouched and no per-kind allow-list is needed.

**The hook ([`MemoryStore`](../../../app/core/memory/memory_store.py)).** `set_flashbulb(provider, *, enabled, max_boost, …)` wires an optional affect provider; `provider()` returns the live `(arousal, episode_intensity)`. Inside `add()`, right after the base salience is clipped, every **non-pinned** write (pinned rows are already special) calls the provider, boosts `salience_clipped`, and — when `charge >= min_charge` (0.05) — injects `metadata.affect_at_encoding = {arousal, episode_intensity, charge, boost}` before the row is encoded + mirrored. The whole block is best-effort (`try/except`, debug-logged) so a broken provider can never break a memory write. MemoryStore stays fully decoupled from AffectState / K57 — reading affect is the caller's concern.

**Wiring ([`SessionController`](../../../app/core/session/session_controller.py) + [`post_turn_helpers_mixin.py`](../../../app/core/session/post_turn_helpers_mixin.py)).** After the MemoryStore is constructed, the controller calls `set_flashbulb(self._read_encoding_affect, …)` with the `memory.flashbulb_*` knobs. `_read_encoding_affect()` reads `_affect_store.get(user_id).arousal` + reuses the existing K68 `_peak_emotion_intensity()` (decayed strongest K57 episode), falling back to a neutral `(0.4, 0.0)` on any failure. A higher initial salience both **surfaces the memory more** (RAG scoring) and gives it **more headroom before decay/prune drops it** — the "burns in / fades slower" property, both halves from one boost.

**Settings.** `memory.flashbulb_enabled` (true) / `_max_boost` (0.35, the most salience a fully-charged moment adds) / `_arousal_weight` (0.6) / `_episode_weight` (0.7) / `_arousal_neutral` (0.4). **MCP** ([`self_state_tools.py`](../../../app/mcp/server_tools/self_state_tools.py)): `get_flashbulb_state` dumps the knobs, whether the hook is wired on the live store, the current live `(arousal, episode_intensity)`, and a preview of the charge + boost a memory written *right now* would get (off base 0.5). Repro: send an emotionally charged message → `get_flashbulb_state` shows the boost climb; the next extracted memory carries `affect_at_encoding`. No persona copy — the mechanic is invisible, it only changes which memories resurface.

**Tests:** [`tests/test_flashbulb.py`](../../../tests/test_flashbulb.py) (pure `compute_charge` neutral/below/high/episode/clamped + `apply_flashbulb` neutral/charged/clamped, plus the `MemoryStore.add` hook: charged write boosts + stamps, neutral write untouched + no stamp, disabled hook no-op, pinned skips, broken provider never breaks the write), `test_flashbulb_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).

---

## K74. Humor-style calibration — what kind of funny lands

K48 tease-rhythm governs the *budget* (how much snark) and K59 the *economy* (payback timing), but nothing tracked **which kind of humor** lands for this user — puns vs dry/deadpan vs absurdist vs self-deprecating vs playful-roast. K74 mirrors the J11 `affection_style` learner exactly: a per-user weighting over a small humour-kind taxonomy on `kv_meta`, learned passively, so over time Aiko reaches for the register *this* person laughs at.

**Pure module ([`humor_style.py`](../../../app/core/relationship/humor_style.py)).** Taxonomy `pun / deadpan / absurdist / self_deprecating / playful_roast`. `classify_turn_humor(text, reaction)` runs cheap per-kind regexes (self-deprecating self-knocks, second-person roast jabs, absurdist hyperbole markers, explicit pun tells) and — crucially — **returns `[]` on a non-funny turn** so learning is sparse and correct (unlike affection, where every turn carries *some* care). A humour-signalling `[[reaction:]]` (playful / smug / amused / …) with no overt text marker is read as **deadpan** (dry delivery has no lexical tell). The rest mirrors J11: `engagement_to_signal`, `apply_observation` (floored + renormalised, "bias never collapse"), `apply_reaction_confirmation` (boosts whichever kinds Aiko *just used*, since a 😂 confirms the register, not a fixed map), `decay_toward_uniform`, `serialize`/`deserialize` (garbage → uniform), `top_kind`.

**Learning wiring.** [`post_turn_mixin.py`](../../../app/core/session/post_turn_mixin.py) runs a two-pass hook right beside J11's: pass 1 attributes the just-observed K14 engagement to the previous turn's `_prev_humor_kinds`; pass 2 tags this turn via `classify_turn_humor` and stashes it. [`world_mixin.py`](../../../app/core/session/world_mixin.py) `apply_user_reaction` adds the sparse confirmation booster: a `laugh` / `eyeroll` reaction confirms the stored `_prev_humor_kinds`. [`humor_style_worker.py`](../../../app/core/relationship/humor_style_worker.py) `HumorStyleDecayWorker` (registered beside the affection one in [`speaking_workers_init_mixin.py`](../../../app/core/session/speaking_workers_init_mixin.py)) is the only path that moves weights *toward* uniform.

**Effect (design decision).** Unlike J11 — where touch / tease / appreciation have concrete code *gates* to multiply a willingness-bias against — there is **no deterministic humour-register selector** in the code path; a strictly "never rendered" learner would therefore be a no-op. So K74's learned top register surfaces as a short suffix on the **existing K48 tease-rhythm cue only** (`_humor_register_hint` in [`inner_life_part1.py`](../../../app/core/session/inner_life_part1.py), gated to fire only when the top register sits `>= humor_style_hint_min_rel × uniform`). It rides a cue that already fires when humour is in play — it never introduces a new always-on narrated block. The persona's tease-rhythm block frames it as a soft nudge ("keep your range — a one-note comedian gets old").

**Settings.** `agent.humor_style_enabled` / `_learning_rate` (0.04) / `_reaction_weight` (0.06) / `_floor` (0.05) / `_decay_half_life_days` (30) / `_hint_min_rel` (1.25) / `_decay_interval_seconds` (21600). **MCP** ([`self_state_tools.py`](../../../app/mcp/server_tools/self_state_tools.py)): `get_humor_style_state` (weights + top kind + register-hint preview + prev-turn kinds + settings), `set_humor_style(kind, weight)`, `reset_humor_style`, `force_humor_style_decay`. Grep `humor-style attribute:` / `humor-style confirm:` / `humor_style decayed:`.

**Tests:** [`tests/test_humor_style.py`](../../../tests/test_humor_style.py) (classify each kind + deadpan-fallback + no-humour-empty + multi-marker, signal bands, observation lift + floor-never-zeroes, reaction confirmation + empty-noop, decay toward uniform, register hint silent-near-uniform / fires-above-threshold, serde round-trip + garbage), `test_humor_style_round_trip` in [`tests/test_settings.py`](../../../tests/test_settings.py).
...
**MCP-debuggable**: `get_wants_state()` (live wants with pressure/age, cooldown map, `cue_preview`, settings), `force_want(text, kind, pressure)`, `force_want_imperative()` (one-shot threshold bypass), `clear_wants()`. Repro: `force_want("ask Jacob about the garden", pressure=0.8)` → `send_message(skip_tts=true)` → the imperative lands in `get_last_response_detail.system_prompt`; mention the garden in the next message and `tail_logs(module_contains="post_turn")` shows the acted line. Settings: 10 `agent.wants_*` knobs. Tests: [`tests/test_wants_ledger.py`](../../../tests/test_wants_ledger.py) (serialisation, add/dedup/cap, growth/expiry/cooldown, acted detection, render bands, brush-off, feeder worker with fakes, assembler slot ordering + aggressive drop).

**Correction — the pressure mechanic never ran (H28).** Measured three months on: every live want sat at 0.150–0.159 and the imperative band had rendered **zero** times, because the feeder's liveness check asked the cue pool for *pending* seeds. A seed leaves `pending` the moment K9's block renders it — and that block spends two a turn — so the median seed left the shelf **1.9 hours** after birth and its want was pruned with it, while pressure needs 19 hours to reach K54's bar and 53 to reach this one. Of 131 seeds ever surfaced, **none** survived 53 hours. The pruner now tests the seed against `CueStore.live()` (the non-terminal states), so a want dies when its topic actually came up rather than when Aiko was merely shown it. Because unblocking pressure alone would turn the imperative into a ratchet — same directive every turn on a topic she keeps declining — an imperative that surfaces and is not acted on now costs the want `wants_brush_off_decay` (0.6) and drops out below `wants_brush_off_floor` (0.2), which is K89's decaying-stake answer applied one layer up. See [`health.md`](../shipped/health.md#h28-a-different-block-spent-k52s-fuel-and-three-features-starved).

**Second correction — the drain moved rather than closing, and the ledger had a monopolist (H29).** A day later the strongest want had reached 0.25 and the oldest was 10.6 hours old: still nothing within a day of either bar. `live()` was right about surfacing and still wrong about **expiry** — a seed expires by being *offered and refused*, and all 110 expired seeds on the install hit `max_surfacings` at exactly two showings, a median 2.9 hours in. The pruner now asks `CueStore.resolved_ids()`, which returns only the `used` / `superseded` subset of the ids the ledger already holds, so an expired seed leaves its want behind to age on `wants_max_age_days`. Asking by *presence* also removes H28's page-full guard: an empty result, a failed read and an oversized ledger now all retire nothing, by construction. Separately, `wants_cap` refuses rather than evicts and the seed feeder refills a freed slot within minutes, so **7 of 8 slots were curiosity seeds** and a goal or pursuit could essentially never claim one — the new `wants_per_source_cap` (4) stops any single source owning more than half the ledger. See [`health.md`](../shipped/health.md#h29-a-want-cannot-outlive-two-showings-of-its-source-cue).
...
**Correction — one of the two arming paths was dead (H28).** `thread_ownership_block` rendered 0 times in 253 instrumented turns. A thread is opened by a K53 initiative turn *or* a K52 imperative want, and the imperative had never fired at all, so the feature ran on half its intended supply. Fixed upstream in K52; nothing here changed.
...
**Corrections — two of the gates were unreachable.** The lull leg was fixed in [H17](../health.md) (it compared `last_mean` against a raw constant that sat below every observed value; it now reads the detector's adaptive band). The offer leg was the other one: `appetite_min_want_pressure=0.35` is 19 hours of K52 pressure growth, and no want survived two, so `decide` returned `no_offer` indefinitely — the block rendered 0 times in 253 instrumented turns. Fixed upstream in K52 ([H28](../shipped/health.md#h28-a-different-block-spent-k52s-fuel-and-three-features-starved)); the bar itself was correctly sited and is unchanged.
...
**Storage is the cue pool** ([`docs/cue-pool.md`](../../cue-pool.md)), as `cue_type="tease_ledger"`. It shipped on its own `kv_meta` key with hand-written cap, expiry, `offered_at` stamp, settle pass and re-offer-on-miss; all five were the pool's own machinery reinvented, and all five went. A debt is a `pending` row, an offer is `mark_surfaced`, a miss is `release()`, the fortnight is `ttl_hours=336`, and a collection is ordinary stage-A matching (`lexical`, `min_overlap=3`). Two behaviour changes came free and were wanted: a collected debt becomes `used` rather than being deleted (so it feeds `get_cue_outcomes` and still never re-collects, since `pending()` excludes it), and a debt offered but never landed retires at `max_surfacings=3` instead of coming round every 12 h for two weeks — looser than the default 2 because "no opening? skip it" makes a miss the *expected* outcome here.

Two pool defaults were wrong for this cue and are now knobs. Its rows want to be **stale** — the gap between banking and collecting is the whole joke — so it is the only type setting `CuePolicy.pick_order=oldest`, and `hold_hours` on `CueStore.add()` seals a fresh debt for `agent.tease_min_age_hours=1`. And the **subject is the user's quote**, not the cue's `what`, which is a constant on the K29 lane ("they pushed back hard on a take of yours", every time): keyed on it each pushback would have superseded the last, and matched on it any reply with "pushed"/"back"/"hard" would have settled an unrelated debt.

**What stayed in the module** ([`tease_ledger.py`](../../../app/core/relationship/tease_ledger.py), ~110 lines from ~330): `subject_for` (which half of a debt is actually about something), `is_duplicate` (three shared content words via `echo_detector.tokens`, because the pool supersedes on an *exact* subject and a reworded quote is the same grudge), and `render_block`, now the cue line alone — its rails moved into the hoisted `Collecting on the ledger:` note.

**Wiring**: two bank triggers — K29 opinion-pushback fire ([`inner_life_part3.py`](../../../app/core/session/inner_life_part3.py), both the definite and resolve paths, each passing the bare quote as `subject`) and the K57 lane-picker (light miffed < 0.35 → ledger, not sulk) — both through `_bank_tease_debt` → `_queue_pool_cue`. Collection: `_render_tease_collection_block` walks master switch → the two gates no policy field expresses (humor axis ≥ `agent.tease_min_humor=0.2`; the offer cooldown `agent.tease_collect_cooldown_hours=12` **divided by the J11 `teasing` affection-style bias**, spent against `store.last_surfaced_at` rather than a kv stamp) → `take_pool_cue`. Lands in **T6 directly after `topic_appetite_block`** and is dropped under `aggressive` (same posture as wants). Persona is split: the concept and the in-the-moment "noted, that's going in the ledger" quip stay in `The ledger:` (banking is available on any turn), while the collection rails hoist into `conditional_handling.txt` and ship only when the block renders.

**MCP-debuggable**: `get_tease_ledger()` (the pool page + per-type stats + cadence, plus the humor/cooldown gates), `force_tease_debt(what, context)`, `force_tease_collection()` (one-shot bypass of humor + cooldown — *not* of the `not_before` seal, which lives on the row; set `tease_min_age_hours=0` to test the loop in one sitting), `clear_tease_ledger()` (expires rather than deletes). Grep targets: `tease banked:` / `tease collection offered:` (INFO); collections show up as `cue used` with `cue_type=tease_ledger`. Settings: 4 `agent.tease_*` knobs — `tease_cap` and `tease_expiry_days` retired to the policy. Tests: [`tests/test_tease_ledger.py`](../../../tests/test_tease_ledger.py) (subject + dedupe math, banking incl. the seal and the two-pushbacks-are-two-debts case, the collection gates and oldest-first pick, the K57 lane-picker, assembler slot).
