...
**Consumer ([`inner_life_part2.py`](../../../app/core/session/inner_life_part2.py) `_render_shared_ritual_block`).** Picks the strongest un-acknowledged ritual, gated by a per-acknowledgment wall-clock cooldown (`shared_ritual.last_surfaced_at`, `agent.shared_ritual_surface_cooldown_days`) so a burst of newly-qualified rituals doesn't announce back-to-back; on fire it flags the ritual `acknowledged` (so it never re-announces — becomes a light standing reference) and stamps the watermark. **T6, right after `wellbeing_concern_block`** (the cue-producer family), retained under aggressive (cooldown/ack-consuming one-shot). **NOT** in the K16 grounding-line suppression set.
