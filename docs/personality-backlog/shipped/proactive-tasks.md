...
**file_write.** `FileWriteHandler` (`HANDLER_FILE_WRITE`) — `write` / `append` / `replace` inside a **writable** root (`read_only=false`), with writable-root gating, extension allow-list, byte cap, and **atomic write** (temp + `os.replace`). Destructive = modifies an existing file; creating a new file is non-destructive (no approval). Reachable **only** as the `write_file` workflow skill ([`skill_registry.py`](../../../app/core/tasks/workflow/skill_registry.py), gated on `agent.file_write.enabled` + a writable root), never a fast brain tool. Nested resource config `FileWriteSettings` (`enabled` / `max_bytes` / `allowed_extensions`); the *approval* is the generic `task_approval_*`. **Later moved out of core into the bundled `filesystem` plugin** (`plugins/filesystem/`), alongside `file_read` / `file_search`, so the handler module and its test no longer live in the core tree — see [`docs/plugins.md`](../../plugins.md). The approval framework above stayed in core, which is the point of it.

**calculate.** AST-whitelisted `safe_eval` (numeric literals, `+ - * / // % **`, unary signs, a `math` allow-list + `abs`/`round`/`min`/`max`, `pi`/`e`/`tau`; no `eval`, no names, no attribute access, bounded exponentiation), synchronous — exact result in the same turn. **Later moved out of core into the bundled `calculator` plugin** (`plugins/calculator/`, via the ToolPlugin SDK `register_fast_tool`); it no longer has a `tools.calculate` flag and the evaluator now lives in the plugin-local `aiko_calc` package. See [`docs/plugins.md`](../../plugins.md) (fast tools).
...
Settings: `agent.task_approval_mode` (`ask`), `agent.task_approval_overrides` (`{}`), `agent.file_write.{enabled=false, max_bytes=262144, allowed_extensions}` (calculate toggled via the `calculator` plugin's `enabled` since the move). Tests: [`tests/test_calculator_plugin.py`](../../../tests/test_calculator_plugin.py) (post-move; was `test_calc_safe_eval.py`), [`tests/test_task_approval.py`](../../../tests/test_task_approval.py), `tests/test_file_write_handler.py` (create/overwrite/append/replace, gating, approval pause→approve/deny/approve-all, skill gating, `_wait_child` wait-through / real-timeout / parent-cancel — moved out with the handler), `TaskApprovalAndFileWriteSettingsTests` in [`tests/test_settings.py`](../../../tests/test_settings.py).
